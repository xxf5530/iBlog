...
const aniClassName = ref<'tile-merge' | 'tile-popup' | undefined>('tile-popup')

watch(
  () => props.level,
  (_, oldValue) => {
    aniClassName.value = oldValue ? 'tile-merge' : 'tile-popup'
  },
)
</script>
...
<template>
  <div class="absolute rounded transition">
    <div
      ...
      :class="[aniClassName, color]"
      @animationend="aniClassName = undefined"
    >
      {{ label }}
    </div>
  </div>
...
.tile-128 {
  ...
  background: #edd073;
  box-shadow:
    0 0 30px 10px rgba(243, 215, 116, 0.238095),
...
.tile-512 {
  ...
  background: #edc950;
  box-shadow:
    0 0 30px 10px rgba(243, 215, 116, 0.396825),
...
.tile-2048 {
  ...
  background: #edc22e;
  box-shadow:
    0 0 30px 10px rgba(243, 215, 116, 0.555556),
