...
const header = computed(() => {
  let ctime = ''
  let mtime = ''
  const { title, date, tags, customHeader, lastUpdateTime } = frontmatter.value
  if (customHeader) {
    if (date) {
      ctime =
        customHeader === 'center'
...
<template>
  <div
    v-if="header.show"
    class="flex flex-col gap-5 py-4 mb-6"
    :class="header.show === 'center' && 'items-center'"
  >
    <h1 class="text-3xl text-$vp-c-brand-1 font-bold">{{ header.title }}</h1>
    <div class="flex flex-wrap gap-x-6 gap-y-2 text-sm text-$vp-c-text-2">
      <div class="flex items-center" title="本文新建时间">
        <span i-tabler-clock mr-1 v-if="header.show !== 'center'" />
      ...
      <div class="flex items-center" v-if="header.tags">
        <span
          mr-1
          :class="header.tags.length > 1 ? 'i-tabler-tags' : 'i-tabler-tag'"
        />
        <template v-for="(tag, idx) in header.tags">
          <a
            class="hover:text-$vp-c-brand-1 underline underline-solid underline-offset-2"
            :href="getLinkByNoteTag(tag)"
            >{{ tag }}</a
          >
          <span class="ml-0.5 mr-1.5" v-if="idx < header.tags.length - 1"
            >,</span
          >
        </template>
      </div>
