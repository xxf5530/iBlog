...
<template>
  <div class="border rounded of-hidden">
    <div
      v-if="state === 'loading'"
      class="min-h-28 bg-black/5 dark:bg-white/10"
    />
    <div v-else-if="state === 'ok'" class="p-4">
      ...
      >
        <img :src="resp.avatar_url" class="h-6 w-6 rounded-full of-hidden" />
        <span>{{ endPoint }}</span>
      </a>
      <div class="mb-4 max-w-full break-all text-sm">
        {{ resp.description }}
      </div>
      <div class="flex flex-wrap gap-x-4 gap-y-2 text-xs text-$vp-c-text-2">
        ...
        <div class="flex items-center" title="stars">
          <span class="i-tabler-star mr-1" />
          {{ resp.stargazers_count }}
        </div>
        <div class="flex items-center" title="forks">
          <span class="i-tabler-git-fork mr-1" />
          {{ resp.forks_count }}
        </div>
        <div v-if="!resp.stamp">
          Last PR on {{ formatTimeAgo(new Date(resp.pushed_at)) }}
        </div>
        <div v-else class="ml-auto">
          This data was cached {{ formatTimeAgo(new Date(resp.stamp)) }}
        </div>
      </div>
    </div>
  </div>
