...
// need "cleanUrls: true" in vitepress config
...
export function getLinkByNoteView(view: NotesView, tag: string) {
  const url = view !== 'tags' ? `${notesPathPrefix}view=${view}` : getLinkByNoteTag(tag)
...
export function getNotesByTags(source: MarkdownArr) {
  ...
  return Object.fromEntries(Object.entries(raw).sort((a, b) => b[1].length - a[1].length))
}
